...
/**
 * Performance test for the CCPhoneNum entity.
 */
public class CCPhoneNumGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cCPhoneNums").get("/api/cc-phone-nums").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cCPhoneNum")
                        .post("/api/cc-phone-nums")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"phoneNum\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cCPhoneNum_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cCPhoneNum").get("${new_cCPhoneNum_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created cCPhoneNum").delete("${new_cCPhoneNum_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CCPhoneNum entity").exec(scn);
