...
/**
 * Performance test for the CIPhoneNum entity.
 */
public class CIPhoneNumGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cIPhoneNums").get("/api/ci-phone-nums").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cIPhoneNum")
                        .post("/api/ci-phone-nums")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"phoneNum\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cIPhoneNum_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cIPhoneNum").get("${new_cIPhoneNum_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created cIPhoneNum").delete("${new_cIPhoneNum_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CIPhoneNum entity").exec(scn);
