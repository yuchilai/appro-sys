...
/**
 * Performance test for the WorkEntry entity.
 */
public class WorkEntryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all workEntries").get("/api/work-entries").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new workEntry")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_workEntry_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created workEntry").get("${new_workEntry_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created workEntry").delete("${new_workEntry_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the WorkEntry entity").exec(scn);
