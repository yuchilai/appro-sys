 ...
 */
public class AddressGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all addresses").get("/api/addresses").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new address")
                        .post("/api/addresses")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created address").get("${new_address_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created address").delete("${new_address_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Address entity").exec(scn);
