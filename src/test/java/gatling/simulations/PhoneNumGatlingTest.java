...
/**
 * Performance test for the PhoneNum entity.
 */
public class PhoneNumGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all phoneNums").get("/api/phone-nums").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new phoneNum")
                        .post("/api/phone-nums")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"phoneNum\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_phoneNum_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created phoneNum").get("${new_phoneNum_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created phoneNum").delete("${new_phoneNum_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PhoneNum entity").exec(scn);
