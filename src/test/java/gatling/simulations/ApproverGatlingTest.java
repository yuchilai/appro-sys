...
/**
 * Performance test for the Approver entity.
 */
public class ApproverGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all approvers").get("/api/approvers").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new approver")
                        .post("/api/approvers")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"signature\": null" + ", \"assignedDate\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_approver_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created approver").get("${new_approver_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created approver").delete("${new_approver_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Approver entity").exec(scn);
