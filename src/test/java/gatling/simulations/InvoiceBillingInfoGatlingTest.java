...
/**
 * Performance test for the InvoiceBillingInfo entity.
 */
public class InvoiceBillingInfoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all invoiceBillingInfos")
                    .get("/api/invoice-billing-infos")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new invoiceBillingInfo")
                        .post("/api/invoice-billing-infos")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_invoiceBillingInfo_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created invoiceBillingInfo").get("${new_invoiceBillingInfo_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created invoiceBillingInfo").delete("${new_invoiceBillingInfo_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the InvoiceBillingInfo entity").exec(scn);
