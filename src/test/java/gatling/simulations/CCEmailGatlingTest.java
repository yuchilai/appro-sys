...
/**
 * Performance test for the CCEmail entity.
 */
public class CCEmailGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cCEmails").get("/api/cc-emails").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cCEmail")
                        .post("/api/cc-emails")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"email\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cCEmail_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cCEmail").get("${new_cCEmail_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created cCEmail").delete("${new_cCEmail_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CCEmail entity").exec(scn);
