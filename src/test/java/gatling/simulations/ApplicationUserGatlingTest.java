...
public class ApplicationUserGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all applicationUsers").get("/api/application-users").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .repeat(5)
                .on(
                    exec(http("Get created applicationUser").get("${new_applicationUser_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created applicationUser").delete("${new_applicationUser_url}").headers(headers_http_authenticated))
                .pause(10)
        );
