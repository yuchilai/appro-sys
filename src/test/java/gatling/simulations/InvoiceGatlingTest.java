 ...
 */
public class InvoiceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all invoices").get("/api/invoices").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new invoice")
                        .post("/api/invoices")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"amount\": 0" +
                                ", \"status\": \"CREATED\"" +
                                ", \"paymentDueDate\": \"2020-01-01T00:00:00.000Z\"" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created invoice").get("${new_invoice_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created invoice").delete("${new_invoice_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Invoice entity").exec(scn);
