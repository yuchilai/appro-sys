...
/**
 * Performance test for the CIEmail entity.
 */
public class CIEmailGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cIEmails").get("/api/ci-emails").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cIEmail")
                        .post("/api/ci-emails")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"email\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cIEmail_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cIEmail").get("${new_cIEmail_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created cIEmail").delete("${new_cIEmail_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CIEmail entity").exec(scn);
