...
/**
 * Performance test for the HourlyRate entity.
 */
public class HourlyRateGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all hourlyRates").get("/api/hourly-rates").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new hourlyRate")
                        .post("/api/hourly-rates")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"name\": \"SAMPLE_TEXT\"" + ", \"rate\": 0" + ", \"isDefault\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_hourlyRate_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created hourlyRate").get("${new_hourlyRate_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created hourlyRate").delete("${new_hourlyRate_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the HourlyRate entity").exec(scn);
