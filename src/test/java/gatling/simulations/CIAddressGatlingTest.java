...
/**
 * Performance test for the CIAddress entity.
 */
public class CIAddressGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cIAddresses").get("/api/ci-addresses").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cIAddress")
                        .post("/api/ci-addresses")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cIAddress_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cIAddress").get("${new_cIAddress_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created cIAddress").delete("${new_cIAddress_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CIAddress entity").exec(scn);
