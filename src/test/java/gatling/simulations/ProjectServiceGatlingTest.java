...
/**
 * Performance test for the ProjectService entity.
 */
public class ProjectServiceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all projectServices").get("/api/project-services").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new projectService")
                        .post("/api/project-services")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_projectService_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created projectService").get("${new_projectService_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created projectService").delete("${new_projectService_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ProjectService entity").exec(scn);
