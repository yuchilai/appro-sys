...
/**
 * Performance test for the Approval entity.
 */
public class ApprovalGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all approvals").get("/api/approvals").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new approval")
                        .post("/api/approvals")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"approved\": null" +
                                ", \"approvalDateTime\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"comments\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_approval_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created approval").get("${new_approval_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created approval").delete("${new_approval_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Approval entity").exec(scn);
