...
/**
 * Performance test for the CCAddress entity.
 */
public class CCAddressGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cCAddresses").get("/api/cc-addresses").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cCAddress")
                        .post("/api/cc-addresses")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cCAddress_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cCAddress").get("${new_cCAddress_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created cCAddress").delete("${new_cCAddress_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CCAddress entity").exec(scn);
